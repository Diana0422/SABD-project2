...
public class Query2V2 extends Query<Query2Record, Query2Result> {

    public Query2V2(String url) {
        this.url = url;
    }
    ...
    public SingleOutputStreamOperator<Query2Result> queryConfiguration(SingleOutputStreamOperator<Query2Record> stream, WindowEnum windowAssigner, String opName) {
        return stream.keyBy(Query2Record::getLocation) // group by location
                ...
                .aggregate(new RankAggregate(), new RankWindowAllFunction()) // compute the top5 and bottom5 for the single window, for each partition
        ...
        if (KAFKA_SINK_ENABLED) {
            ...
            var sink = KafkaSink.<Query2Result>builder()
                    ...
                    .setRecordSerializer(KafkaRecordSerializationSchema.builder()
                            .setTopic("query2V2-" + windowType.name())
