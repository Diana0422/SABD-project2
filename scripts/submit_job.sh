...
then
  ...
  docker exec -t -i jobmanager flink run -p "$2" -c com.diagiac.flink.query2.Query2V2 ./sensor-app/sabd2-flink-1.0-jar-with-dependencies.jar kafka://kafka:9092
elif [[ $1 -eq 3 ]]
then
  echo "Submitting query 3 to Flink"
  docker exec -t -i jobmanager flink run -p "$2" -c com.diagiac.flink.query3.Query3V2 ./sensor-app/sabd2-flink-1.0-jar-with-dependencies.jar kafka://kafka:9092
elif [[ $1 -eq 4 ]]
then
